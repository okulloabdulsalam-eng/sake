 ...
 * Supports both form data (Google Drive) and JSON input (GitHub Storage)
 */

require_once __DIR__ . '/library_media_config.php';

// Only require Google API if available
$googleApiAvailable = file_exists(__DIR__ . '/../media-storage/vendor/autoload.php');
...
// Check if JSON input (GitHub storage) or form data (Google Drive)
$contentType = $_SERVER['CONTENT_TYPE'] ?? '';
$isJsonInput = strpos($contentType, 'application/json') !== false;

if ($isJsonInput) {
    // JSON input from GitHub storage
    ...
    $password = $data['password'] ?? $_SERVER['HTTP_X_ADMIN_PASSWORD'] ?? '';
    ...
    $directDownloadLink = $data['download_url'] ?? null;
    $driveFileId = $data['storage_path'] ?? $data['github_sha'] ?? null;
    $fileSize = $data['file_size'] ?? 0;
    $bookFileName = $data['file_name'] ?? null;
    ...
    // Skip Google Drive upload for JSON input - go straight to database
    goto save_to_database;
}

// Form data input (original Google Drive flow)
$title = $_POST['title'] ?? '';
$author = $_POST['author'] ?? '';
$isbn = $_POST['isbn'] ?? '';
...
// Label for goto from JSON input path
save_to_database:
