...
function initOfflineReminderBootstrap() {
    try {
        if (!window.Capacitor || !window.Capacitor.isNativePlatform || !window.Capacitor.isNativePlatform()) return;
        if (window.KiumaOfflineReminders && typeof window.KiumaOfflineReminders.init === 'function') {
            window.KiumaOfflineReminders.init();
            return;
        }
        if (document.querySelector('script[data-kiuma-offline-reminders="1"]')) return;
        var script = document.createElement('script');
        script.src = 'js/offline-reminders.js';
        script.defer = true;
        script.setAttribute('data-kiuma-offline-reminders', '1');
        ...
        console.warn('[OfflineReminders] Bootstrap failed:', e);
    }
}

if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initOfflineReminderBootstrap);
} else {
    initOfflineReminderBootstrap();
}
...
if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => {
        navigator.serviceWorker.register('./sw.js')
            .then(reg => {
                ...
                requestOfflineCacheRefresh(reg);
    ...
    window.addEventListener('online', () => {
        navigator.serviceWorker.getRegistration('./sw.js').then((reg) => {
            if (reg) requestOfflineCacheRefresh(reg);
        }).catch(() => {});
    });
...
// Close side menu when a nav link is clicked (so menu pushes back after selecting a page)
document.addEventListener('click', function(e) {
    if (e.target.closest('#navMenu .nav-link')) {
        if (navMenu) navMenu.classList.remove('active');
        if (overlay) overlay.classList.remove('active');
    }
});

// Top mini bar — show page name in hero-nav (SAKE design)
document.addEventListener('DOMContentLoaded', function initHeroNavPageTitle() {
    const span = document.getElementById('heroNavPageTitle');
    if (!span) return;
    const titleEl = document.querySelector('.hero-page-title');
    if (titleEl) {
        span.textContent = titleEl.textContent.trim();
    } else {
        const h1 = document.querySelector('.hero-compact-title h1, main h1, .page-content h1');
        span.textContent = (h1 && h1.textContent.trim()) ? h1.textContent.trim() : (document.title || 'KIUMA').replace(/\s*[-|]\s*KIUMA\s*$/i, '').trim() || 'KIUMA';
    }
});
...
(function initStickyMiniBar() {
    ...
    if (document.getElementById('quranApp')) return;
...
async function updateDates() {
    ...
    // 1) Gregorian: set immediately (sync) so it never loads late
    var dateDisplay = document.getElementById('dateDisplay');
    if (dateDisplay) {
        dateDisplay.textContent = now.toLocaleDateString('en-US', dateOptions);
    }

    // 2) Hijri: show something immediately (cache or local calc), then optionally refresh from API
    var hijriDateEl = document.getElementById('hijriDate');
    if (!hijriDateEl) return;

    var hijriDay, hijriMonth, hijriYear;
    var cached = getCachedHijriDate();
    if (cached) {
        hijriDay = cached.day;
    ...
    if (navigator.onLine) {
        try {
            var queryDate = new Date(now.getTime());
            var dd = String(queryDate.getDate()).padStart(2, '0');
            var mm = String(queryDate.getMonth() + 1).padStart(2, '0');
            var yyyy = queryDate.getFullYear();
            var apiUrl = 'https://api.aladhan.com/v1/gToH/' + dd + '-' + mm + '-' + yyyy;
            var controller = new AbortController();
    ...
    // White days check (use current hijriDay/hijriMonth/hijriYear)
    if (hijriDay >= 13 && hijriDay <= 15 && hijriMonth && hijriYear) {
...
function checkAndCreateWhiteDaysNotification(hijriDay, hijriMonth, hijriYear, hijriMonths) {
    ...
    const notification = {
        ...
        message: `Assalaam Alaikum! Today is the ${hijriDay}${hijriDay === 1 ? 'st' : hijriDay === 2 ? 'nd' : hijriDay === 3 ? 'rd' : 'th'} of ${monthName} ${hijriYear} AH, one of the blessed White Days (Ayyam al-Beed). Fasting on the 13th, 14th, and 15th is highly recommended (Sunnah). May Allah accept your good deeds.`,
        icon: 'fas fa-moon',
        status: 'unread',
        date: new Date().toISOString(),
...
function checkAndCreateFastingReminder() {
    ...
    const greeting = userName ? `Assalaam Alaikum ${userName}!` : 'Assalaam Alaikum!';
...
// Parse time string to minutes since midnight (24h). Handles "05:40", "5:40", "16:00", "4:30 PM"
function parseTimeToMinutes(timeStr) {
    if (!timeStr || typeof timeStr !== 'string') return NaN;
    var s = timeStr.trim();
    var match = s.match(/^\s*(\d{1,2})\s*:\s*(\d{1,2})\s*(?::\s*\d{1,2})?\s*(AM|PM)?\s*$/i);
    if (!match) return NaN;
    var hours = parseInt(match[1], 10);
    var minutes = parseInt(match[2], 10);
    if (match[3]) {
        var pm = (match[3].toUpperCase() === 'PM');
...
function loadPrayerTimes() {
    ...
    prayerNames.forEach(function(name) {
        var adhanEl = document.getElementById(name + 'Adhan');
        if (adhanEl) {
            var rawTime = adhanEl.textContent.trim();
            prayerTimes.push({
                name: name.charAt(0).toUpperCase() + name.slice(1),
                id: name,
                time: rawTime,
                timeMinutes: parseTimeToMinutes(rawTime),
    ...
    for (var i = 0; i < prayerTimes.length; i++) {
        ...
        var prayerMinutes = prayer.timeMinutes;
...
function normalizeUserData(rawUser) {
    ...
    const fallbackName = (user.name || '').toString().trim();
    ...
    return {
        ...
        firstName: firstName,
        lastName: lastName,
        name: fullName || fallbackName || 'User',
        email: (user.email || '').toString().trim(),
        phone: (user.phone || user.phoneNumber || '').toString().trim(),
        regNo: (user.regNo || user.registrationNumber || user.regNumber || '').toString().trim(),
        whatsapp: (user.whatsapp || user.whatsApp || user.whatsappNumber || '').toString().trim(),
...
function getStoredUserData() {
    try {
        ...
        if (userData) return normalizeUserData(JSON.parse(userData));
    } catch (e) {}

    try {
        const fallbackUser = localStorage.getItem('kiuma_user');
        if (fallbackUser) return normalizeUserData(JSON.parse(fallbackUser));
    } catch (e) {}
...
function renderAccountInfo(user) {
    ...
    const accountGenderEl = document.getElementById('accountGender');
    const accountPositionEl = document.getElementById('accountPosition');
    const accountPhoneEl = document.getElementById('accountPhone');
    const accountRegNoEl = document.getElementById('accountRegNo');
    const accountWhatsAppEl = document.getElementById('accountWhatsApp');

    if (accountNameEl) accountNameEl.textContent = safeUser.name || 'Not provided';
    if (accountEmailEl) accountEmailEl.textContent = safeUser.email || 'Not provided';
    if (accountGenderEl) accountGenderEl.textContent = safeUser.gender || 'Not specified';
    if (accountPositionEl) accountPositionEl.textContent = safeUser.position || 'Not specified';
    if (accountPhoneEl) accountPhoneEl.textContent = safeUser.phone || 'Not provided';
    if (accountRegNoEl) accountRegNoEl.textContent = safeUser.regNo || 'Not provided';
    if (accountWhatsAppEl) accountWhatsAppEl.textContent = safeUser.whatsapp || 'Not provided';
}
            ...
            <div class="modal-body">
                ...
                <div id="accountInfo" style="display: none;">
                    <div class="account-info-card">
                        ...
                        <p style="text-align: center; color: var(--text-gray); margin-bottom: 10px;">
                            <i class="fas fa-phone"></i> <span id="accountPhone"></span>
                        </p>
                        <p style="text-align: center; color: var(--text-gray); margin-bottom: 10px;">
                            <i class="fas fa-id-card"></i> <span id="accountRegNo"></span>
                        </p>
                        <p style="text-align: center; color: var(--text-gray); margin-bottom: 10px;">
                            <i class="fab fa-whatsapp"></i> <span id="accountWhatsApp"></span>
                        </p>
                        <p style="text-align: center; color: var(--text-gray); margin-bottom: 20px;">
...
window.handleLogin = async function(e) {
    ...
    try {
        ...
        const baseUserData = normalizeUserData({
            uid: firebaseUser.uid,
            email: firebaseUser.email,
            name: firebaseUser.displayName || email.split('@')[0]
        });
        currentUser = baseUserData;
...
window.showEditProfile = function() {
    ...
    let user = currentUser || getStoredUserData();
    ...
    const safeUser = normalizeUserData(user);
    if (firstNameEl) firstNameEl.value = safeUser.firstName || '';
    if (lastNameEl) lastNameEl.value = safeUser.lastName || '';
    if (phoneEl) phoneEl.value = safeUser.phone || '';
    if (regNoEl) regNoEl.value = safeUser.regNo || '';
    if (whatsAppEl) whatsAppEl.value = safeUser.whatsapp || '';
    if (positionEl) positionEl.value = safeUser.position || '';
...
function updateNotificationBadge() {
    ...
    const displayText = unreadCount > 0 ? (unreadCount > 99 ? '99+' : String(unreadCount)) : '';
    const showBadge = unreadCount > 0;
    // Hero bar and any .notifications-btn badges
    ...
    // Sticky mini-bar and any [data-notification-badge] (same system notifications count)
    const standaloneBadges = document.querySelectorAll('[data-notification-badge], .sticky-mini-bar .smb-btn .badge');
