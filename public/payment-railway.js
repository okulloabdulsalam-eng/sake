...
async function processPayment(paymentData) {
  ...
  // Step 2: Store reference, order tracking ID, and payer info for verification and receipt
  if (typeof sessionStorage !== 'undefined') {
    ...
    var name = paymentData.payer_name || [paymentData.first_name, paymentData.last_name].filter(Boolean).join(' ').trim() || paymentData.email || 'Payer';
    sessionStorage.setItem('pesapal_payer_name', name);
    sessionStorage.setItem('pesapal_category', paymentData.category || paymentData.description || 'Payment');
    sessionStorage.setItem('pesapal_amount', String(paymentData.amount || ''));
    sessionStorage.setItem('pesapal_currency', paymentData.currency || 'UGX');
  }
