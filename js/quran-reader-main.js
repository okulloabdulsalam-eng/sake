...
(function () {
  ...
  function renderMushafPageHtml(page, verses) {
    ...
    return (
      ...
      toArabicIndic(page) +
      ' —</div><p class="qr-print-hint">Mushaf-style lines (Madinah script images via <a href="https://quran.com" class="qr-link" target="_blank" rel="noopener">Quran.com</a> API). For comparison, <a href="https://github.com/mBilG/Open-Mushaf" class="qr-link" target="_blank" rel="noopener">Open-Mushaf</a> (Linux/GTK) shows full-page PNGs from local <code>001.png</code>…<code>604.png</code> packs — not shipped in this web app. Requires internet.</p></div>'
    );
  }
  ...
  async function loadMushafPage(page) {
    ...
    if (!body) return;
    body.innerHTML = '<div class="qr-skel">Loading mushaf page…</div>';
    mushafAyahs = [];
    var all = [];
    var p = 1;
    try {
      ...
      if (!withImg.length) {
        body.innerHTML = '<p class="qr-muted">No line images for this page. Check connection or try adaptive mode.</p>';
        return;
      }
      ...
      body.innerHTML = '<p class="qr-muted">Could not load mushaf page. Try adaptive mode or check connection.</p>';
    }
  }
  ...
  window.qrSetMode = function (v) {
    ...
    if (sel) sel.value = v;
    if (v === 'mushaf') openMushafPage(curPage || SURAH_PAGE[(curSurah || 1) - 1]);
    else {
      showView('qrReader');
