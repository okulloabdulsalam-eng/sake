...
$workers = @("storage-kiuma4", "video1", "video2", "video3", "video4", "video5", "audio", "library")
...
foreach ($w in $workers) {
    ...
    if (Test-Path (Join-Path $dir "wrangler.toml")) {
        Write-Host "--- Deploying $w ---" -ForegroundColor Yellow
        Push-Location $dir
        try {
            npx wrangler deploy
            Write-Host "  OK: $w deployed" -ForegroundColor Green
        } catch {
            Write-Host "  FAIL: $w - $_" -ForegroundColor Red
        }
        Pop-Location
    } else {
