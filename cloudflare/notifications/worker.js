...
async function checkHijriReminders(env) {
  ...
  const hijri = await getHijriDate(env, now);
  if (!hijri) return;
...
async function getHijriDate(env, now = new Date()) {
  try {
    ...
    if (!response.ok) {
      console.error('[Hijri] Aladhan API error:', response.status);
      return null;
    }
    ...
    console.error('[Hijri] Date lookup error:', err.message);
    return null;
  }
}
